...
public class OttoPDF
{
  ...
  public static enum PDFType { QUOTATION,  APPROVAL,  PRODUCTIONAPPROVAL,  GENERIC,  ARTWORK,  DIGITIZING,  EMBSAMPLE,  HEATSAMPLE,  CADSAMPLE,  SCREENSAMPLE,  DIRECTTOGARMENTSAMPLE,  EMBORDER,  HEATORDER,  CADORDER,  SCREENORDER,  DIRECTTOGARMENTORDER,  OVERSEASQUOTATION,  OVERSEASAPPROVAL,  OVERSEASPRODUCTIONAPPROVAL,  OVERSEASGENERIC,  OVERSEASARTWORK,  OVERSEASDIGITIZING,  OVERSEASSAMPLE,  OVERSEASPURCHASEORDER,  PATCHORDER,  PATCHSAMPLE;
  }
  ...
  public OttoPDF(HttpServletRequest request, HttpServletResponse response, SQLTable sqltable) throws Exception
  {
    ...
    this.domesticpricecalc = new DomesticPriceCalculation(sqltable, this.therealorder);
    this.overseaspricecalc = new OverseasPriceCalculation(sqltable, this.therealorder);
  ...
  public void directtogarmentorder() throws Exception {
    this.domesticpricecalc.calculateOrderPrice(DomesticPriceCalculation.DirectToGarment);
  ...
  public void heatorder() throws Exception {
    this.domesticpricecalc.calculateOrderPrice(DomesticPriceCalculation.HeatTransfer);
  ...
  public void cadorder() throws Exception {
    this.domesticpricecalc.calculateOrderPrice(DomesticPriceCalculation.CADPrint);
    
    if (!this.isIE) {
      this.response.setHeader("Content-disposition", "attachment; filename=\"" + new SimpleDateFormat("MM.dd.yy-hh.mm.a-").format(new Date()) + "DomesticWorkOrderS" + this.therealorder.getVendorInformation().getCADPrintVendor().getWorkOrderNumber() + ".pdf\"");
    }
    setHeaderColor(OrderColor);
    setOrderType(PDFType.CADORDER);
    PdfWriter writer = PdfWriter.getInstance(this.document, this.response.getOutputStream());
    writer.setPageEvent(new PageNumbers());
    this.document.open();
  ...
  public void screenorder() throws Exception {
    this.domesticpricecalc.calculateOrderPrice(DomesticPriceCalculation.ScreenPrint);
  ...
  public void patchorder() throws Exception {
    this.domesticpricecalc.calculateOrderPrice(DomesticPriceCalculation.Patch);
  ...
  public void emborder() throws Exception {
    this.domesticpricecalc.calculateOrderPrice(DomesticPriceCalculation.Embroidery);
  ...
  public void directtogarmentsampleorder() throws Exception {
    this.domesticpricecalc.calculateSamplePrice(DomesticPriceCalculation.DirectToGarment);
  ...
  public void heatsampleorder() throws Exception {
    this.domesticpricecalc.calculateSamplePrice(DomesticPriceCalculation.HeatTransfer);
  ...
  public void cadsampleorder() throws Exception {
    this.domesticpricecalc.calculateSamplePrice(DomesticPriceCalculation.CADPrint);
    
    if (!this.isIE) {
      this.response.setHeader("Content-disposition", "attachment; filename=\"" + new SimpleDateFormat("MM.dd.yy-hh.mm.a-").format(new Date()) + "DomesticSampleWorkOrderS" + this.therealorder.getVendorInformation().getCADPrintVendor().getWorkOrderNumber() + ".pdf\"");
    }
    setHeaderColor(OrderColor);
    setOrderType(PDFType.CADSAMPLE);
    PdfWriter writer = PdfWriter.getInstance(this.document, this.response.getOutputStream());
    writer.setPageEvent(new PageNumbers());
    this.document.open();
  ...
  public void screensampleorder() throws Exception {
    this.domesticpricecalc.calculateSamplePrice(DomesticPriceCalculation.ScreenPrint);
  ...
  public void patchsampleorder() throws Exception {
    this.domesticpricecalc.calculateSamplePrice(DomesticPriceCalculation.Patch);
  ...
  public void embsampleorder() throws Exception {
    this.domesticpricecalc.calculateSamplePrice(DomesticPriceCalculation.Embroidery);
  ...
  public void overseassampleorder(int overseasvendornumber) throws Exception {
    this.overseassamplevendorcalc = new OverseasVendorFreeProductSampleCalculator(this.therealorder, this.sqltable);
  ...
  private void productpreviewPage(ExtendOrderData currentorder) throws Exception {
    for (int i = 0; i < currentorder.getSetCount(); i++) {
      if (getOrderType() == PDFType.PRODUCTIONAPPROVAL) {
      ...
      } else if ((getOrderType() == PDFType.EMBSAMPLE) || (getOrderType() == PDFType.HEATSAMPLE) || (getOrderType() == PDFType.CADSAMPLE) || (getOrderType() == PDFType.SCREENSAMPLE) || (getOrderType() == PDFType.DIRECTTOGARMENTSAMPLE) || (getOrderType() == PDFType.PATCHSAMPLE)) {
        boolean containsemb = false;
        boolean containsheat = false;
        boolean containscad = false;
        ...
        boolean containscadswatch = false;
        ...
        for (int j = 0; j < currentorder.getSet(i).getLogoCount(); j++) {
          ...
          } else if (currentorder.getSet(i).getLogo(j).getDecoration().equals("CAD Print")) {
            containscad = true;
            if ((currentorder.getSet(i).getLogo(j).getSwatchToDo() != null) && (currentorder.getSet(i).getLogo(j).getSwatchToDo().intValue() > 0)) {
              containscadswatch = true;
            }
        ...
        } else if (getOrderType() == PDFType.CADSAMPLE) {
          vendornumber = currentorder.getVendorInformation().getCADPrintVendor().getVendor().intValue();
        ...
        if ((vendornumber == currentorder.getVendorInformation().getEmbroideryVendor().getVendor().intValue()) && ((containsembswatch) || ((containsemb) && (currentorder.getSet(i).getItem(0).getProductSampleToDo() != null) && (currentorder.getSet(i).getItem(0).getProductSampleToDo().intValue() > 0)))) {
          ...
          this.document.newPage();
          this.document.add(logopreview(currentorder, i));
          ...
          this.document.newPage();
          this.document.add(logopreview(currentorder, i));
        } else if ((vendornumber == currentorder.getVendorInformation().getCADPrintVendor().getVendor().intValue()) && ((containscadswatch) || ((containscad) && (currentorder.getSet(i).getItem(0).getProductSampleToDo() != null) && (currentorder.getSet(i).getItem(0).getProductSampleToDo().intValue() > 0)))) {
          this.document.newPage();
          this.document.add(productpreviewitem(currentorder, i));
      ...
      } else if ((getOrderType() == PDFType.EMBORDER) || (getOrderType() == PDFType.CADORDER) || (getOrderType() == PDFType.SCREENORDER) || (getOrderType() == PDFType.HEATORDER) || (getOrderType() == PDFType.DIRECTTOGARMENTORDER) || (getOrderType() == PDFType.PATCHORDER)) {
        boolean containsemb = false;
        boolean containsheat = false;
        boolean containscad = false;
        boolean containsscreen = false;
        ...
        if (getOrderType() == PDFType.EMBORDER) {
          ...
          vendornumber = currentorder.getVendorInformation().getHeatTransferVendor().getVendor().intValue();
        } else if (getOrderType() == PDFType.CADORDER) {
          vendornumber = currentorder.getVendorInformation().getCADPrintVendor().getVendor().intValue();
        ...
        if ((currentorder.getVendorInformation().getEmbroideryVendor().getVendor().intValue() == vendornumber) && (containsemb)) {
          ...
          this.document.newPage();
          this.document.add(logopreview(currentorder, i));
        } else if ((currentorder.getVendorInformation().getCADPrintVendor().getVendor().intValue() == vendornumber) && (containscad)) {
          this.document.newPage();
          this.document.add(productpreviewitem(currentorder, i));
  ...
  private PdfPTable productpreviewitem(ExtendOrderData currentorder, int item) throws Exception {
    ...
    if ((getOrderType() == PDFType.DIGITIZING) || (getOrderType() == PDFType.EMBSAMPLE) || (getOrderType() == PDFType.HEATSAMPLE) || (getOrderType() == PDFType.CADSAMPLE) || (getOrderType() == PDFType.SCREENSAMPLE) || (getOrderType() == PDFType.DIRECTTOGARMENTSAMPLE) || (getOrderType() == PDFType.PATCHSAMPLE) || (getOrderType() == PDFType.EMBORDER) || (getOrderType() == PDFType.HEATORDER) || (getOrderType() == PDFType.CADORDER) || (getOrderType() == PDFType.SCREENORDER) || (getOrderType() == PDFType.DIRECTTOGARMENTORDER) || (getOrderType() == PDFType.PATCHORDER) || (getOrderType() == PDFType.OVERSEASDIGITIZING) || (getOrderType() == PDFType.OVERSEASSAMPLE) || (getOrderType() == PDFType.OVERSEASPURCHASEORDER)) {
  ...
  private PdfPTable logopreview(ExtendOrderData currentorder, int item) throws Exception
  {
    PdfPTable mytable = new PdfPTable(1);
    mytable.setTotalWidth(this.document.getPageSize().getWidth() - this.document.leftMargin() - this.document.rightMargin());
    mytable.setWidthPercentage(100.0F);
    ...
    PdfPCell billinformationcell = headercell("LOGO PREVIEW", this.headerfont);
    ...
    if ((getOrderType() == PDFType.PRODUCTIONAPPROVAL) && ((currentorder.getSet(item).getItem(0).getProductSampleTotalDone() == null) || (currentorder.getSet(item).getItem(0).getProductSampleTotalDone().intValue() == 0))) {
      for (int i = 0; i < currentorder.getSet(item).getLogoCount(); i++) {
        if ((currentorder.getSet(item).getLogo(i).getSwatchTotalDone() != null) && (currentorder.getSet(item).getLogo(i).getSwatchTotalDone().intValue() > 0)) {
          PdfPCell mytablecell = paddedTableCell(logopreviewPanel(currentorder, item, i), 0);
      ...
      for (int i = 0; i < currentorder.getSet(item).getLogoCount(); i++) {
        ...
        if ((vendornumber == currentorder.getVendorInformation().getEmbroideryVendor().getVendor().intValue()) && ((currentorder.getSet(item).getLogo(i).getDecoration().equals("Flat Embroidery")) || (currentorder.getSet(item).getLogo(i).getDecoration().equals("3D Embroidery"))) && (((currentorder.getSet(item).getItem(0).getProductSampleToDo() != null) && (currentorder.getSet(item).getItem(0).getProductSampleToDo().intValue() > 0)) || ((currentorder.getSet(item).getLogo(i).getSwatchToDo() != null) && (currentorder.getSet(item).getLogo(i).getSwatchToDo().intValue() > 0)))) {
          ...
          body.addCell(mytablecell);
          body.addCell(buzzcell("", this.cellfont));
        } else if ((vendornumber == currentorder.getVendorInformation().getCADPrintVendor().getVendor().intValue()) && (currentorder.getSet(item).getLogo(i).getDecoration().equals("CAD Print")) && (((currentorder.getSet(item).getItem(0).getProductSampleToDo() != null) && (currentorder.getSet(item).getItem(0).getProductSampleToDo().intValue() > 0)) || ((currentorder.getSet(item).getLogo(i).getSwatchToDo() != null) && (currentorder.getSet(item).getLogo(i).getSwatchToDo().intValue() > 0)))) {
          PdfPCell mytablecell = paddedTableCell(logopreviewPanel(currentorder, item, i), 0);
          mytablecell.setPaddingBottom(10.0F);
          body.addCell(mytablecell);
          body.addCell(buzzcell("", this.cellfont));
    ...
    } else if ((getOrderType() == PDFType.EMBORDER) || (getOrderType() == PDFType.HEATORDER) || (getOrderType() == PDFType.CADORDER) || (getOrderType() == PDFType.SCREENORDER) || (getOrderType() == PDFType.DIRECTTOGARMENTORDER) || (getOrderType() == PDFType.PATCHORDER)) {
      for (int i = 0; i < currentorder.getSet(item).getLogoCount(); i++) {
        int vendornumber = 0;
        if (getOrderType() == PDFType.EMBORDER) {
          vendornumber = currentorder.getVendorInformation().getEmbroideryVendor().getVendor().intValue();
        ...
        if ((currentorder.getVendorInformation().getEmbroideryVendor().getVendor() != null) && (currentorder.getVendorInformation().getEmbroideryVendor().getVendor().intValue() == vendornumber) && ((currentorder.getSet(item).getLogo(i).getDecoration().equals("Flat Embroidery")) || (currentorder.getSet(item).getLogo(i).getDecoration().equals("3D Embroidery")))) {
          ...
          body.addCell(mytablecell);
          body.addCell(buzzcell("", this.cellfont));
        } else if ((currentorder.getVendorInformation().getCADPrintVendor().getVendor() != null) && (currentorder.getVendorInformation().getCADPrintVendor().getVendor().intValue() == vendornumber) && (currentorder.getSet(item).getLogo(i).getDecoration().equals("CAD Print"))) {
          PdfPCell mytablecell = paddedTableCell(logopreviewPanel(currentorder, item, i), 0);
          mytablecell.setPaddingBottom(10.0F);
          body.addCell(mytablecell);
          body.addCell(buzzcell("", this.cellfont));
          ...
          body.addCell(mytablecell);
          body.addCell(buzzcell("", this.cellfont));
        } else if ((currentorder.getVendorInformation().getEmbroideryVendor().getVendor() != null) && (currentorder.getVendorInformation().getEmbroideryVendor().getVendor().intValue() == vendornumber) && (!currentorder.getSet(item).getLogo(i).getDecoration().equals("Flat Embroidery")) && (!currentorder.getSet(item).getLogo(i).getDecoration().equals("3D Embroidery")) && (!currentorder.getSet(item).getLogo(i).getDecoration().equals("Heat Transfer")) && (!currentorder.getSet(item).getLogo(i).getDecoration().equals("CAD Print")) && (!currentorder.getSet(item).getLogo(i).getDecoration().equals("Screen Print")) && (!currentorder.getSet(item).getLogo(i).getDecoration().equals("Four Color Screen Print")) && (!currentorder.getSet(item).getLogo(i).getDecoration().equals("Direct To Garment"))) {
          PdfPCell mytablecell = paddedTableCell(logopreviewPanel(currentorder, item, i), 0);
          mytablecell.setPaddingBottom(10.0F);
          body.addCell(mytablecell);
          body.addCell(buzzcell("", this.cellfont));
    ...
    PdfPCell mylogocell = paddedTableCell(headerBodyPanel(header, body), 0);
    mylogocell.setMinimumHeight(this.document.getPageSize().getHeight() - this.document.bottomMargin() - this.document.topMargin() - mytable.getHeaderHeight());
    mytable.addCell(mylogocell);
    
    return mytable;
  }
  
  private PdfPTable logopreviewPanel(ExtendOrderData currentorder, int item, int logo) throws Exception {
    PdfPTable info = new PdfPTable(2);
    info.setSplitLate(false);
    float[] infoWidths = { 30.0F, 70.0F };
    info.setWidths(infoWidths);
    
    int maximageheight = 325;
    int maximagewidth = 390;
    int imgtimes = 2;
    ...
    if (!currentorder.getSet(item).getLogo(logo).getFilename().equals("")) {
      ...
      if (currentorder.getSet(item).getLogo(logo).getFilename().toLowerCase().endsWith(".dst")) {
        String samedstimage = "bigimage" + currentorder.getSet(item).getLogo(logo).getFilename() + currentorder.getSet(item).getLogo(logo).getThreadBrand();
        ...
        logopreview = this.sameimages.getBiggerDSTImage(samedstimage, currentorder, currentorder.getSet(item).getLogo(logo), this.sqltable, maximagewidth * imgtimes, maximageheight * imgtimes);
      } else {
        logopreview = this.sameimages.getImage("bigimage" + currentorder.getSet(item).getLogo(logo).getFilename(), "C:\\WorkFlow\\NewWorkflowData\\" + currentorder.getOrderTypeFolder() + "Data/" + currentorder.getHiddenKey() + "/logos/" + currentorder.getSet(item).getLogo(logo).getFilename(), maximagewidth * imgtimes, maximageheight * imgtimes);
      }
      
      logopreview.scaleToFit(maximagewidth, maximageheight);
      ...
      if ((currentorder.getOrderType() == OrderData.OrderType.OVERSEAS) && (!currentorder.getSet(item).getLogo(logo).getDstFilename().equals(""))) {
        String samedstimage = "bigimage" + currentorder.getSet(item).getLogo(logo).getDstFilename() + currentorder.getSet(item).getLogo(logo).getThreadBrand();
        for (int i = 0; i < currentorder.getSet(item).getLogo(logo).getColorwayCount(); i++) {
          samedstimage = samedstimage + currentorder.getSet(item).getLogo(logo).getColorway(i).getConvertCode();
        }
        Image logopreview = this.sameimages.getBiggerDSTImage(samedstimage, currentorder, currentorder.getSet(item).getLogo(logo), this.sqltable, maximagewidth * imgtimes, maximageheight * imgtimes);
        logopreview.scaleToFit(maximagewidth, maximageheight);
        logopicture = new PdfPCell(logopreview, false);
      } else {
    ...
    logopicture.setMinimumHeight(maximageheight);
    ...
    if ((getOrderType() == PDFType.EMBSAMPLE) || (getOrderType() == PDFType.HEATSAMPLE) || (getOrderType() == PDFType.CADSAMPLE) || (getOrderType() == PDFType.SCREENSAMPLE) || (getOrderType() == PDFType.DIRECTTOGARMENTSAMPLE) || (getOrderType() == PDFType.PATCHSAMPLE) || (getOrderType() == PDFType.OVERSEASSAMPLE)) {
      String extrainfo = "";
  ...
  private PdfPTable orderinformationPanel(ExtendOrderData currentorder, int item) throws Exception {
    ...
    } else if ((getOrderType() == PDFType.CADSAMPLE) || (getOrderType() == PDFType.CADORDER)) {
      info.addCell(buzzcell("Work Order Vendor:", this.cellfontbold));
      info.addCell(buzzcell(currentorder.getVendorInformation().getCADPrintVendor().getVendorName(), this.cellfont));
    ...
    if (getOrderType() == PDFType.DIGITIZING) {
      ...
      info.addCell(buzzcell("Processing Date:", this.cellfontbold));
      info.addCell(buzzcell(fixdate(currentorder.getVendorInformation().getCADPrintVendor().getWorkOrderProcessingDate()), this.cellfont));
      
      info.addCell(buzzcell("Due Date:", this.cellfontbold));
      info.addCell(buzzcell(fixdate(currentorder.getVendorInformation().getCADPrintVendor().getWorkOrderDueDate()), this.cellfont));
    } else if ((getOrderType() == PDFType.SCREENSAMPLE) || (getOrderType() == PDFType.SCREENORDER)) {
      info.addCell(buzzcell("Processing Date:", this.cellfontbold));
    ...
    if (getOrderType() == PDFType.OVERSEASDIGITIZING) {
      ...
      } else if ((getOrderType() == PDFType.HEATORDER) || (getOrderType() == PDFType.CADORDER) || (getOrderType() == PDFType.SCREENORDER) || (getOrderType() == PDFType.EMBORDER) || (getOrderType() == PDFType.DIRECTTOGARMENTORDER) || (getOrderType() == PDFType.PATCHORDER)) {
        info2.addCell(buzzcell("Total Order Cost:", this.cellfontbold));
  ...
  private PdfPTable previewitemnotesPanel(ExtendOrderData currentorder, int item) {
    ...
    if ((currentorder.getSet(item).getItem(0).getHasPrivatePackaging() != null) && (currentorder.getSet(item).getItem(0).getHasPrivatePackaging().booleanValue())) {
      privatelabeltext = privatelabeltext + "Custom Packaging - See Attached Instructions\n";
    }
    
    if ((currentorder.getSet(item).getItem(0).getHasPrivateShippingMark() != null) && (currentorder.getSet(item).getItem(0).getHasPrivateShippingMark().booleanValue())) {
      privatelabeltext = privatelabeltext + "Custom Shipping Mark - See Attached Instructions\n";
    }
  ...
  private PdfPTable previewlogoPanel(ExtendOrderData currentorder, int item, int logo) throws Exception {
    ...
    if (!currentorder.getSet(item).getLogo(logo).getFilename().equals("")) {
      ...
      if (currentorder.getSet(item).getLogo(logo).getFilename().toLowerCase().endsWith(".dst")) {
        ...
        logopreview = this.sameimages.getDSTImage(samedstimage, currentorder, currentorder.getSet(item).getLogo(logo), this.sqltable, 300, 180);
      } else {
      ...
      if ((currentorder.getOrderType() == OrderData.OrderType.OVERSEAS) && (!currentorder.getSet(item).getLogo(logo).getDstFilename().equals(""))) {
        ...
        Image logopreview = this.sameimages.getDSTImage(samedstimage, currentorder, currentorder.getSet(item).getLogo(logo), this.sqltable, 300, 180);
        logopreview.scaleToFit(100.0F, 60.0F);
        logopicture = new PdfPCell(logopreview, false);
      } else {
        logopicture = new PdfPCell(new Phrase(" ", this.cellfontbold));
  ...
  private PdfPTable previewcolorwayPanel(ExtendOrderData currentorder, int item, int logo) throws Exception
  {
    if (((currentorder.getSet(item).getLogo(logo).getFilename() != null) && (currentorder.getSet(item).getLogo(logo).getFilename().toLowerCase().endsWith(".dst"))) || ((currentorder.getSet(item).getLogo(logo).getDstFilename() != null) && (currentorder.getSet(item).getLogo(logo).getDstFilename().toLowerCase().endsWith(".dst")))) {
      ...
      colorwaytable.addCell(buzzcell("Logo Description", this.cellfontbold));
    ...
    if ((currentorder.getSet(item).getLogo(logo).getDecoration() != null) && (currentorder.getSet(item).getLogo(logo).getDecoration().equals("CAD Print"))) {
    ...
    if ((currentorder.getSet(item).getLogo(logo).getDecoration() != null) && ((currentorder.getSet(item).getLogo(logo).getDecoration().equals("Screen Print")) || (currentorder.getSet(item).getLogo(logo).getDecoration().equals("Four Color Screen Print")))) {
      PdfPTable colorwaytable = new PdfPTable(7);
      colorwaytable.setSplitLate(false);
      float[] colorwaytableWidths = { 5.0F, 15.0F, 10.0F, 17.0F, 13.0F, 20.0F, 20.0F };
      colorwaytable.setWidths(colorwaytableWidths);
    ...
    throws Exception
  {
    ...
    if ((getOrderType() == PDFType.DIGITIZING) || (getOrderType() == PDFType.EMBORDER) || (getOrderType() == PDFType.HEATORDER) || (getOrderType() == PDFType.CADORDER) || (getOrderType() == PDFType.SCREENORDER) || (getOrderType() == PDFType.DIRECTTOGARMENTORDER) || (getOrderType() == PDFType.PATCHORDER) || (getOrderType() == PDFType.OVERSEASDIGITIZING) || (getOrderType() == PDFType.OVERSEASPURCHASEORDER))
    {
        ...
        for (int i = 0; i < currentorder.getSet(item).getLogoCount(); i++) {
          if ((getOrderType() == PDFType.OVERSEASSAMPLE) && (currentorder.getSet(item).getLogo(i).getSwatchToDo() != null)) {
            ...
            if ((currentorder.getVendorInformation().getEmbroideryVendor().getVendor() != null) && (vendornumber == currentorder.getVendorInformation().getEmbroideryVendor().getVendor().intValue()) && (currentorder.getSet(item).getLogo(i).getSwatchToDo() != null) && ((currentorder.getSet(item).getLogo(i).getDecoration().equals("Flat Embroidery")) || (currentorder.getSet(item).getLogo(i).getDecoration().equals("3D Embroidery")))) {
              totalswatchs += currentorder.getSet(item).getLogo(i).getSwatchToDo().intValue();
            } else if ((currentorder.getVendorInformation().getCADPrintVendor().getVendor() != null) && (vendornumber == currentorder.getVendorInformation().getCADPrintVendor().getVendor().intValue()) && (currentorder.getSet(item).getLogo(i).getSwatchToDo() != null) && (currentorder.getSet(item).getLogo(i).getDecoration().equals("CAD Print"))) {
              totalswatchs += currentorder.getSet(item).getLogo(i).getSwatchToDo().intValue();
    ...
    } else if ((getOrderType() == PDFType.EMBSAMPLE) || (getOrderType() == PDFType.HEATSAMPLE) || (getOrderType() == PDFType.CADSAMPLE) || (getOrderType() == PDFType.SCREENSAMPLE) || (getOrderType() == PDFType.DIRECTTOGARMENTSAMPLE) || (getOrderType() == PDFType.PATCHSAMPLE) || (getOrderType() == PDFType.EMBORDER) || (getOrderType() == PDFType.HEATORDER) || (getOrderType() == PDFType.SCREENORDER) || (getOrderType() == PDFType.DIRECTTOGARMENTORDER) || (getOrderType() == PDFType.PATCHORDER))
    {
      if ((getOrderType() == PDFType.EMBORDER) || (getOrderType() == PDFType.HEATORDER) || (getOrderType() == PDFType.CADORDER) || (getOrderType() == PDFType.SCREENORDER) || (getOrderType() == PDFType.DIRECTTOGARMENTORDER) || (getOrderType() == PDFType.PATCHORDER)) {
    ...
    if ((getOrderType() == PDFType.OVERSEASPURCHASEORDER) || (getOrderType() == PDFType.OVERSEASPRODUCTIONAPPROVAL)) {
      ...
      if ((currentorder.getSet(item).getItem(0).getHasPrivatePackaging() != null) && (currentorder.getSet(item).getItem(0).getHasPrivatePackaging().booleanValue())) {
        info.addCell(buzzcell("Custom Packaging - See Attached Instructions", this.cellfontbold));
        info.addCell(buzzcell("", this.cellfont));
      } else {
        info.addCell(buzzcell("OTTO Standard Packaging", this.cellfontbold));
        info.addCell(buzzcell("", this.cellfont));
      }
      if ((currentorder.getSet(item).getItem(0).getHasPrivateShippingMark() != null) && (currentorder.getSet(item).getItem(0).getHasPrivateShippingMark().booleanValue())) {
        info.addCell(buzzcell("Custom Shipping Mark - See Attached Instructions", this.cellfontbold));
        info.addCell(buzzcell("", this.cellfont));
      } else {
        info.addCell(buzzcell("OTTO Standard Shipping Mark", this.cellfontbold));
        info.addCell(buzzcell("", this.cellfont));
      }
    }
    
    if ((getOrderType() == PDFType.OVERSEASPURCHASEORDER) || (getOrderType() == PDFType.EMBORDER) || (getOrderType() == PDFType.HEATORDER) || (getOrderType() == PDFType.CADORDER) || (getOrderType() == PDFType.SCREENORDER) || (getOrderType() == PDFType.DIRECTTOGARMENTORDER) || (getOrderType() == PDFType.PATCHORDER)) {
  ...
  private PdfPTable termsPanel() throws Exception { PdfPTable header = new PdfPTable(1);
    ...
    if ((!this.termran) && (this.therealorder.getCustomerInformation().getEclipseAccountNumber() != null) && (this.sqltable.makeTable("SELECT * FROM `eclipse_special_terms` WHERE `eclipseaccount` =" + this.therealorder.getCustomerInformation().getEclipseAccountNumber() + " LIMIT 1").booleanValue())) {
      String tempterm_quotation = ((String)this.sqltable.getCell("quotation")).trim();
      if (!tempterm_quotation.equals("")) {
        this.term_quotation = tempterm_quotation;
      }
      String tempterm_approval = ((String)this.sqltable.getCell("approval")).trim();
      if (!tempterm_approval.equals("")) {
        this.term_approval = tempterm_approval;
      }
      String tempterm_overseasquotation = ((String)this.sqltable.getCell("overseasquotation")).trim();
      if (!tempterm_overseasquotation.equals("")) {
        this.term_overseas_quotation = tempterm_overseasquotation;
      }
      String tempterm_overseasapproval = ((String)this.sqltable.getCell("overseasapproval")).trim();
  ...
  private PdfPTable mainheaderPanel2(ExtendOrderData currentorder) throws Exception {
    ...
    PdfPCell number;
    PdfPCell number; if ((getOrderType() == PDFType.DIGITIZING) || (getOrderType() == PDFType.EMBSAMPLE) || (getOrderType() == PDFType.HEATSAMPLE) || (getOrderType() == PDFType.CADSAMPLE) || (getOrderType() == PDFType.SCREENSAMPLE) || (getOrderType() == PDFType.DIRECTTOGARMENTSAMPLE) || (getOrderType() == PDFType.PATCHSAMPLE) || (getOrderType() == PDFType.EMBORDER) || (getOrderType() == PDFType.HEATORDER) || (getOrderType() == PDFType.CADORDER) || (getOrderType() == PDFType.SCREENORDER) || (getOrderType() == PDFType.DIRECTTOGARMENTORDER) || (getOrderType() == PDFType.PATCHORDER)) {
    ...
    throws Exception
  {
    ...
    if ((currentorder.getSet(item).getItem(0).getHasPrivatePackaging() != null) && (currentorder.getSet(item).getItem(0).getHasPrivatePackaging().booleanValue())) {
      PdfPCell cell = buzzcell("Custom Packaging - See Attached Instructions", this.cellfontbold);
      cell.setColspan(2);
      itemdesc.addCell(cell);
    } else {
      PdfPCell cell = buzzcell("Otto Standard Packaging", this.cellfontbold);
      cell.setColspan(2);
      itemdesc.addCell(cell);
    }
    
    if ((currentorder.getSet(item).getItem(0).getHasPrivateShippingMark() != null) && (currentorder.getSet(item).getItem(0).getHasPrivateShippingMark().booleanValue())) {
      PdfPCell cell = buzzcell("Custom Shipping Mark - See Attached Instructions", this.cellfontbold);
      cell.setColspan(2);
      itemdesc.addCell(cell);
    } else {
      PdfPCell cell = buzzcell("Otto Standard Shipping Mark", this.cellfontbold);
      cell.setColspan(2);
      itemdesc.addCell(cell);
    }
  ...
  private PdfPTable logoArtworkSummaryPage(ExtendOrderData currentorder, int item, int logo) throws Exception {
    ...
    colorways.addCell(buzzcell("Logo Description", this.cellfontbold));
    ...
    if (!currentorder.getSet(item).getLogo(logo).getFilename().equals("")) { Image logopreview;
      ...
      if (currentorder.getSet(item).getLogo(logo).getFilename().toLowerCase().endsWith(".dst")) {
        ...
        if (getOrderType() == PDFType.ARTWORK) {
          logopreview = this.sameimages.getDSTImageNoResize(samedstimage, currentorder, currentorder.getSet(item).getLogo(logo), this.sqltable);
        } else {
          logopreview = this.sameimages.getDSTImageNoConvert(samedstimage, currentorder, currentorder.getSet(item).getLogo(logo), this.sqltable);
        }
      }
      else {
  ...
  private PdfPTable logoSummaryPage(ExtendOrderDataLogo thelogo, int logopos) throws Exception {
    ...
    if (thelogo.getMetallicThread()) {
      logoserv.addCell(buzzcell("Metallic Thread", this.cellfont));
    }
    if (thelogo.getNeonThread()) {
      logoserv.addCell(buzzcell("Neon Thread", this.cellfont));
    }
